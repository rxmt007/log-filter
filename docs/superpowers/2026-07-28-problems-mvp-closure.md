...
本报告记录 Problems MVP 在功能、并发、10GiB 性能和受控内存方面的收口结果。结论按
同一最终代码、同一合成 corpus 的三轮中位数判断：**当前 macOS 开发机上的实现硬门槛
全部通过**。受限于本机不能可靠清空系统 page cache，冷/暖缓存各三次和与历史版本同工具
的系统级 RSS 对照仍属于环境验证项，不能据此推导所有平台和机器都达到相同数字。
